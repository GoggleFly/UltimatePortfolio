...
final class ExtensionTests: BaseTestCase {
    ...
    func testIssueTagsUnwrap() {
        ...
        issue.addToTags(tag)
        XCTAssertEqual(issue.issueTags.count, 1, "Adding 1 tag to an issue should increase issueTags count to 1.")
    }
    ...
    func testTagActiveIssues() {
        ...
        XCTAssertEqual(tag.tagActiveIssues.count, 1, "A new tag with 1 new issues should have 1 active issue.")
    ...
    func testTagSortingIsStable() {
        ...
        tag2.name = "B Tag"
        tag2.id = UUID(uuidString: "FFFFFFFF-0F1B-4252-91D0-B89DB502AA87")
